...
class PlaceBarChartAdapter :
    ...
    class PlaceListStatsViewHolder(
        ...
        fun bind(placeStatsChartUI: PlaceListStatsChartUI) {
            ...
            chartConfirmed.configure(placeStatsChartUI.stats.map { placeStats -> placeStats.date },
                placeStatsChartUI.stats.minBy { placeStats -> placeStats.confirmed }!!.confirmed)
            chartDeaths.configure(placeStatsChartUI.stats.map { placeStats -> placeStats.date },
                placeStatsChartUI.stats.minBy { placeStats -> placeStats.deaths }!!.deaths)
            chartRecovered.configure(placeStatsChartUI.stats.map { placeStats -> placeStats.date },
                placeStatsChartUI.stats.minBy { placeStats -> placeStats.recovered }!!.recovered)
            chartOpenCases.configure(placeStatsChartUI.stats.map { placeStats -> placeStats.date },
                placeStatsChartUI.stats.minBy { placeStats -> placeStats.openCases }!!.openCases)

            chartConfirmed.setValues(ctx, placeStatsChartUI.stats.map { placeStats ->
                placeStats.confirmed }, R.string.total_confirmed, R.color.dark_red)
            chartDeaths.setValues(ctx, placeStatsChartUI.stats.map { placeStats ->
                placeStats.deaths }, R.string.total_deaths, R.color.dark_grey)
            chartRecovered.setValues(ctx, placeStatsChartUI.stats.map { placeStats ->
                placeStats.recovered }, R.string.total_recovered, R.color.dark_green)
            chartOpenCases.setValues(ctx, placeStatsChartUI.stats.map { placeStats ->
