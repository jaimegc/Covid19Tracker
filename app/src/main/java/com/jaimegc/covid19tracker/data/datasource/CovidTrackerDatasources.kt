...
import com.jaimegc.covid19tracker.domain.model.mapEntity
import com.jaimegc.covid19tracker.domain.model.mapEntityMenuValid
import com.jaimegc.covid19tracker.domain.model.mapEntityValid
import com.jaimegc.covid19tracker.domain.model.toDomain
import com.jaimegc.covid19tracker.domain.model.toEntity
...
import com.jaimegc.covid19tracker.domain.model.toPojoRegionsOrdered
import com.jaimegc.covid19tracker.domain.model.toPojoSubRegionDomain
import com.jaimegc.covid19tracker.domain.model.toPojoSubRegionsOrdered
    ...
    fun getSubRegionsAndStatsWithMostConfirmed(
        ...
        mapEntityMenuValid(subRegionStatsDao.getSubRegionsAndStatsWithMostConfirmed(idCountry, idRegion)) {
            subRegionsListOneStats -> subRegionsListOneStats.toPojoSubRegionsOrdered().let {
                subRegionsListStats -> Pair(
                    subRegionsListStats.isNotEmpty(),
    ...
    suspend fun populateDatabase(covidTrackers: List<CovidTracker>) {
        ...
        covidTrackers.mapIndexed { index, covidTracker ->
            ...
            covidTracker.countriesStats.map { countryStats ->
                if (index == 0) {
                    ...
                    countryStats.regionStats?.map { regionStats ->
                        ...
                        regionStats.subRegionStats?.map { subRegionStats ->
                            subRegionEntities.add(
                                subRegionStats.subRegion.toEntity(
                ...
                countryStats.regionStats?.map { regionStats ->
                    regionStatsEntities.add(
                        regionStats.toEntity(
                            regionStats.region.id,
                            countryStats.country.id
                        )
                    )
