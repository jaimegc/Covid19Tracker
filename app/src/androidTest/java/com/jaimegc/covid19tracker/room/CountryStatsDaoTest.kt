...
class CountryStatsDaoTest : DatabaseTest() {

    private lateinit var countryStats: CountryStatsDao
    ...
    fun setup() {
        initDb()
        countryStats = database.countryStatsDao()
    }
    ...
    fun getAll_shouldReturnCountries() = runBlocking {
        countryStats.getAll().take(1).collect {
    ...
    @Test
    fun getCountryAndStatsByLastDateByCountry_shouldReturnPojo() = runBlocking {
        countryStats.getCountryAndStatsByLastDate(
            country.id
        ).take(1).collect {
    ...
    @Test
    fun getCountryAndStatsByCountryAndDate_shouldReturnPojo() = runBlocking {
        countryStats.getCountryAndStatsByDate(
            country.id,
    ...
    fun getCountriesAndStatsWithMostConfirmed_shouldReturnFiveCountriesMostConfirmedOrderedById() = runBlocking {
        generateCountryAndStats()

        countryStats.getCountriesAndStatsWithMostConfirmed().take(1).collect {
    ...
    fun getCountriesAndStatsWithMostDeaths_shouldReturnFiveCountriesMostDeathsOrderedById() = runBlocking {
        ...
        countryStats.getCountriesAndStatsWithMostDeaths().take(1).collect {
            assertThat(it).hasSize(5)
            assertThat(it[0].country).isEqualTo(australiaPojo.country)
            assertThat(it[0].countryStats).isEqualTo(australiaPojo.countryStats)
    ...
    fun getCountriesAndStatsWithMostOpenCases_shouldReturnFiveCountriesMostOpenCasesOrderedById() = runBlocking {
        ...
        countryStats.getCountriesAndStatsWithMostOpenCases().take(1).collect {
            assertThat(it).hasSize(5)
            assertThat(it[0].country).isEqualTo(australiaPojo.country)
            assertThat(it[0].countryStats).isEqualTo(australiaPojo.countryStats)
            assertThat(it[1].country).isEqualTo(francePojo.country)
    ...
    fun getCountriesAndStatsWithMostRecovered_shouldReturnFiveCountriesMostRecoveredOrderedById() = runBlocking {
        ...
        countryStats.getCountriesAndStatsWithMostRecovered().take(1).collect {
            assertThat(it).hasSize(5)
            assertThat(it[0].country).isEqualTo(australiaPojo.country)
            assertThat(it[0].countryStats).isEqualTo(australiaPojo.countryStats)
            assertThat(it[1].country).isEqualTo(egyptPojo.country)
    ...
    private fun generateCountryAndStats() = runBlocking {
        val germany = countryEntity.copy(id = "germany", name = "Germany", nameEs = "Alemania", "DE")
        val france = countryEntity.copy(id = "france", name = "France", nameEs = "Francia", code = "FR")
        val italy = countryEntity.copy(id = "italy", name = "Italy", nameEs = "Italia", code = "IT")
        val australia = countryEntity.copy(id = "australia", name = "Australia", nameEs = "Australia", code = "AU")
        val egypt = countryEntity.copy(id = "egypt", name = "Egypt", nameEs = "Egipto", code = "EG")
