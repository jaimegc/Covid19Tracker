...
class GetCountryTest : UseCaseTest() {
    ...
    @Test
    fun `get countries should return loading and unknown database error`() = runBlockingTest {
        val flow = flow {
            emit(Either.right(stateListCountryLoading))
    ...
    @Test
    fun `get countries should return loading and unknown database error using flow test`() = runBlockingTest {
        val flow = flow {
            emit(Either.right(stateListCountryLoading))
            emit(Either.left(stateErrorUnknownDatabase))
        }
    ...
    @Test
    fun `get countries should return loading and empty success using flow test`() = runBlockingTest {
        val flow = flow {
            emit(Either.right(stateListCountryLoading))
            emit(Either.right(stateListCountryEmptyData))
        }
